...
| キー | 何が起きるか |
|---|---|
| **`s`** | `shots/` に 3 点セットを保存: 絵の PNG / 設定の .json / **生カメラの `_raw.png`** |
...
`_raw.png` が要点です。**生のカメラ画像があれば、Claude 側で
`--source shots/xxxxxx_raw.png` として同じ画を何度でも描き直せます。**
つまり意匠の追い込みをこちらで回せるようになるので、あなたの往復が減ります。
