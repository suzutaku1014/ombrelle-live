...
Web カメラの実写をそのまま変換したもの (等倍・無加工、[高画質版](docs/images/demo.mp4))。
はじめは静止していて、途中から動く。**止まっている間は筆も止まり、動くと向きが流れに乗る。**

### 30秒でわかる技術的な見どころ

- 単眼深度を筆の大きさ・彩度・空気遠近へ、オプティカルフローを筆触の向きへ接続した
- 推論と描画を非同期化し、遅い推論結果を待たずにリアルタイム描画を続ける構成にした
- 深度モデルを自作の小型モデルへ蒸留し、合成入力でパラメータ数を **25.6分の1**、描画を **52.9 → 134.6 fps** にした

[設計](docs/architecture.md) · [実験記録と未解決課題](docs/devlog.md) · [高画質デモ](docs/images/demo.mp4)
...
> 実写での再学習はまだ。上の精度は合成シーンで採ったもので、汎化性能の主張には使えない。
> 詳細は [docs/devlog.md](docs/devlog.md) の「未着手 / 宿題」。
...
**おまけ: 色彩分割は「平均が元の色に戻る」ことが定義**

輝度を抜いた対立色平面で色度ベクトルを ±Δ 回し、各筆を **1/cos(Δ) 倍**に伸ばすと、
2 つの平均が厳密に元の色へ戻る。結果として **各筆は元より彩度が高い** — これが
「絵具を混ぜるより明るく見える」理由 (混色は彩度を落とすが、並置は落とさない)。
原典の固定色相回転と違い、元の色相が何であっても成立する。

さらに、分離角は**筆の大きさに反比例**させてある。光学混合は「筆が目の中で
混ざる」ことが前提なので、大きい筆で強く分割すると混ざらずに色ノイズになる
(スーラは点、モネは大きめの筆で弱い分割)。連動させることで、
`split` は筆サイズに依らず「揺らめきの量」を意味するツマミになる。
...
| `t` `y` | 色彩分割の強さ (筆の大きさに連動するので単独で意味を持つ) |
| `f` `g` | 中性面への色の注入量 (白い壁を暖色と寒色で描く) |
| `i` `o` | 記憶色(肌)の保護 — 肌の近くの色相だけ振れ幅を抑える |
| `,` `.` | 筆のぼかし (カメラの mip レベル) |
| `-` `=` | 絵の強さ (0 でグレーディングのみ、1 で完全に絵) |
| `4` `5` | 筆の向きが深度にどれだけ従うか |
| `j` | 色の操作を Oklab で行う ↔ luma の対立色平面 (A/B) |
| `a` | 人物と背景の色の関係 (dL / R_C / dh) を測って HUD に出す |
| `x` | 測った彩度比を目標帯へ寄せる閉ループ |
...
「止まっているものが止まって見えない」という指摘に、静止画のスクショでは答えられない。
時間の話は 1 枚の絵では原理的に検証できないので、測る側を用意した。

| キー | |
|---|---|
| `8` | **動きの地図** — 連続フレームを読み戻し、どこがどれだけ動いているかを PNG と数値で出す |
| `9` | **録画** — 描いた結果と**カメラの入力**を対で保存する (設定の .json も) |
| `z` | 入力の時間平滑化 (センサノイズで筆の色が明滅するのを抑える) |
| `;` `'` | フローの不感帯 (実カメラは誰も動かなくても流れ続ける) |
| `6` `7` | 深度の空間ぼかし |

入力を対で残すのが要で、`--source shots/rec-*_raw.mp4` で**同じ場面を設定を変えて
描き直せる**。実際この機能で、「静止した画面での毎フレーム変化」を設定ごとに測り、
既定値を目ではなく数字で決めた (0.61 → 0.43、3 階調を超えて動く画素 1.0% → 0.2%)。
