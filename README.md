...
> **現状の注記**: 下の画像はすべて `--source synthetic` の検証用シーンによるもの。
> 実写カメラでの見栄えの追い込みは未実施 (実行環境のカメラ権限が未付与のため)。
> 奥行きの手がかりと周期的な動きを意図的に入れた合成シーンで、全段の動作は検証済み。
...
| `s` | スクリーンショット (設定を書いた .json も一緒に保存) |
